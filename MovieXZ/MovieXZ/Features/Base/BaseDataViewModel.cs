...
namespace Core
{
    public abstract class BaseDataViewModel<T> : BaseViewModel where T : class
    {
        ...
        {
            var result = await _alertService.ShowAsync(Resource.DefaultDialogTitle, msg, Resource.TryAgain, Resource.DefaultGoBack);
            if (result)
                await LoadDataAsync();
        }
        ...
        protected async Task DisplayAlert(string msg)
            => await _alertService.ShowAsync(Resource.DefaultDialogTitle, msg, Resource.DefaultGoBack);
